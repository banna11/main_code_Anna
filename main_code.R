...
correl <- all_correlations() #dataset for plotting
days <-  as.Date(seq(from = as.Date("2010-02-20"), to = as.Date("2016-12-31"), by = 'day'))
plot <- ggplot(data=correl, aes(as.Date(days))) +
         geom_line(aes( y = name1, colour ="blue")) +
         geom_line(aes( y = name2, colour="red")) +
         geom_line(aes( y = name3, colour="green")) +
         ggtitle("Keresztkorreáció") + xlab("Dátum") + ylab("Érték")
plot
